...
remove() {
  ...
  if [ ${BINARY} == "seid" ]; then
    ...
    sudo rm -rf $HOME/${CONFIG_FOLDER}/data/*
  elif [ ${BINARY} != "seid" ]; then
    echo -e "$YELLOW Resetting chain data...$NORMAL"
...
statesyncOthers() {
  line
  LATEST_HEIGHT=$(curl -s ${RPC}/block | jq -r .block.header.height)
  TRUST_HEIGHT=$((LATEST_HEIGHT - ${BLOCKS}))
  TRUST_HASH=$(curl -s "${RPC}/block?height=$TRUST_HEIGHT" | jq -r .block_id.hash)
  echo -e "$YELLOW RPC SERVERS:$NORMAL$RED ${RPC},${RPC}$NORMAL"
  echo -e "$YELLOW TRUST_HEIGHT:$NORMAL$RED ${TRUST_HEIGHT}$NORMAL"
  echo -e "$YELLOW TRUST_HASH:$NORMAL$RED ${TRUST_HASH}$NORMAL"
  line
  ...
  sed -i.bak -E 's#^(rpc-servers[[:space:]]+=[[:space:]]+).*$#\1"'$RPC','$RPC'"#' $HOME/${CONFIG_FOLDER}/config/config.toml
  sed -i.bak -E 's#^(trust-height[[:space:]]+=[[:space:]]+).*$#\1"'$TRUST_HEIGHT'"#' $HOME/${CONFIG_FOLDER}/config/config.toml
  sed -i.bak -E 's#^(trust-hash[[:space:]]+=[[:space:]]+).*$#\1"'$TRUST_HASH'"#' $HOME/${CONFIG_FOLDER}/config/config.toml

  # STATESYNC enabled
  sed -i.bak -E 's#^(use-p2p[[:space:]]+=[[:space:]]+).*$#\1'true'#' $HOME/${CONFIG_FOLDER}/config/config.toml
  sed -i.bak -E 's#^(enable[[:space:]]+=[[:space:]]+).*$#\1'true'#' $HOME/${CONFIG_FOLDER}/config/config.toml
}
